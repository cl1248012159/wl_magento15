	...
	*/
	public function saveRow( array $importData )
	{
		...
		if ($importData['type'] == 'downloadable' && $importData['downloadable_options'] != "") {
			//start sample_download
			if (isset( $importData['sample_download'])) {
				if ($new) {
					...
					$commadelimiteddata = explode('|',$importData['sample_download']);
					
					foreach ($commadelimiteddata as $data) {
						$configBundleOptionsCodes = $this->userCSVDataAsArray($data);
							
						$downloadableitems['sample'][$downloadableitemsoptionscount]['is_delete'] = 0;
						$downloadableitems['sample'][$downloadableitemsoptionscount]['sample_id'] = 0;
						$downloadableitems['sample'][$downloadableitemsoptionscount]['title'] = $configBundleOptionsCodes[0];
						$downloadableitems['sample'][$downloadableitemsoptionscount]['type'] = 'url';
						$downloadableitems['sample'][$downloadableitemsoptionscount]['sample_url'] = $configBundleOptionsCodes[1];
						$downloadableitems['sample'][$downloadableitemsoptionscount]['sort_order'] = 0;
						$downloadableitems['sample'][$downloadableitemsoptionscount]['file'] = '';
						...
						// 删除已有
/*					  $download_info=Mage::getModel('downloadable/sample');
						$download_info->setProduct($product);
						if ($download_info->hasSamples()) {
							$_samples=$download_info->getUrl();
							foreach ($_samples as $_sample) {
											$_sample->delete();
							}
						}
*/							
						$_samplesCollection = Mage::getModel('downloadable/sample')->getCollection()
						->addProductToFilter($product->getId())
						->addTitleToResult($product->getStoreId());

						foreach ($_samplesCollection as $sample) {
							if($sample->getUrl()) {
							$sample->delete();
							}
						}
			...
			// start link_download
			if (isset( $importData['downloadable_options'])) {
			if ($new) {
			    $downloadableitems = array();
			  	$filearrayforimport = array();
			  	$filenameforsamplearrayforimport = array();
...
if ($finalsuperattributetype == 'grouped') {
		...
		foreach($finalIDssthatneedtobeconvertedto as $data_product_position_id)
		{
			 ...
			 $select_qry_catalog_product_link = "SELECT link_id FROM ".$prefix."catalog_product_link WHERE linked_product_id = '" . trim($data_productID) . "'";
